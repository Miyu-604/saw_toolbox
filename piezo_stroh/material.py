...
class PiezoMaterial:
    ...
    Conventions:
      - C6: stiffness in Voigt (engineering shear), units: Pa
      - e36: piezo tensor in Voigt (engineering shear), units: C/m^2
    ...
    def C4(self) -> np.ndarray:
        return voigtC_to_tensor4(self.C6)

    @property
    def e3(self) -> np.ndarray:
        return voigt_e_to_tensor3(self.e36)
    ...
    def rotated(self, R: np.ndarray, *, name_suffix: str | None = None) -> "PiezoMaterial":
        ...
        Rotate material tensors:
          ...
          eps'   = R eps R^T
        Return rotated material in the same Voigt convention (engineering shear).
        """
        ...
        C6r = tensor4_to_voigtC(C4r)
        e36r = tensor3_to_voigt_e(e3r)

        new_name = self.name if name_suffix is None else f"{self.name}_{name_suffix}"
        return PiezoMaterial(new_name, self.rho, C6r, e36r, epsr)