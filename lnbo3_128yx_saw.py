...
def main():
    ...
    db = MaterialDB()
    ln_voigt = db.get("linbo3", "bulk_ogi2002_present")
    ...
    # --- Stroh solver ---
    solver = PiezoSAWSolver(ln128)

    v_short, err_short = solver.find_velocity(electric_bc="short", vmin=3600, vmax=4200)
    v_open, err_open = solver.find_velocity(electric_bc="open", vmin=3600, vmax=4200)
    ...
    print(f"Calculated K^2: {k2*100:.2f} %")
    ...
    plot_profile(z, prof_open, f"Open surface (v={v_open:.2f} m/s)")
    plot_profile(z, prof_short, f"Short surface (v={v_short:.2f} m/s)")
